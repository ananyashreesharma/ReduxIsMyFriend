...
export default function PlaygroundSection() {
  ...
  return (
    <section id="playground" className="py-24 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-1/2 h-full bg-neutral-900/50 z-0" />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
          <div>
            <h2 className="text-3xl font-bold mb-2">live playground</h2>
            <p className="text-neutral-400">Dispatch stuff and watch state update (the right way). Undo = time travel. You’re welcome.</p>
          </div>
          <div className="flex items-center gap-2 sm:gap-4 bg-neutral-800 p-2 rounded-lg border border-neutral-700 shrink-0">
            <button
              onClick={undo}
              disabled={!canUndo}
              className="p-2 rounded transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40 disabled:opacity-40 disabled:cursor-not-allowed"
              title="undo last move"
            >
              <RotateCcw className={`w-4 h-4 ${canUndo ? 'text-neutral-400 hover:text-white' : 'text-neutral-500'}`} />
            </button>
            <button
              disabled={!canRedo}
              className="p-2 rounded transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40 disabled:opacity-40 disabled:cursor-not-allowed"
            ...
            <span className="text-xs text-neutral-500 px-2 font-mono">time travel</span>
          </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 border border-neutral-700 rounded-xl overflow-hidden shadow-2xl">
          <div className="bg-[#1e2227] p-0 flex flex-col min-h-[500px]">
            <div className="bg-[#282c34] px-4 py-2 flex items-center justify-between border-b border-black/20">
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono text-neutral-400">reducer.js</span>
              </div>
            ...
            <div className="flex-1 p-6 font-mono text-sm leading-relaxed overflow-auto text-neutral-300">
          ...
          <div className="bg-neutral-900 flex flex-col border-l border-neutral-700">
            <div className="flex-1 p-6 bg-neutral-950/50">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-neutral-400 uppercase tracking-wider">current state</h3>
                <span className="text-xs bg-neutral-800 text-green-400 px-2 py-1 rounded border border-neutral-700">valid json</span>
              </div>

              <div
                className={`bg-[#0d1117] p-4 rounded-lg border border-neutral-800 h-64 font-mono text-sm relative overflow-hidden transition-all duration-300 ${jsonFlash ? 'bg-neutral-700' : ''}`}
                id="json-tree"
              >
                <div className="absolute right-4 top-4 text-neutral-600 text-6xl opacity-20 pointer-events-none">
                  <Braces />
                </div>
                <span className="text-purple-400">{'{'}</span>
                <br />
                &nbsp;&nbsp;<span className="text-neutral-400">&quot;balance&quot;</span>: <span className="text-yellow-400" id="display-balance">{state.balance}</span>,<br />
                &nbsp;&nbsp;<span className="text-neutral-400">&quot;loading&quot;</span>: <span className="text-red-400">{state.loading.toString()}</span>,<br />
                &nbsp;&nbsp;<span className="text-neutral-400">&quot;transactions&quot;</span>: <span className="text-neutral-400">[ ]</span>
                <br />
                <span className="text-purple-400">{'}'}</span>
              </div>

              <div className="mt-6 flex items-center justify-center p-6 glass rounded-xl border border-dashed border-neutral-700">
                <div className="text-center">
                  <div className="text-xs text-neutral-500 uppercase mb-2">what you see</div>
            ...
            <div className="p-6 border-t border-neutral-800 bg-neutral-900">
              <h3 className="text-sm font-bold text-neutral-400 uppercase tracking-wider mb-4">dispatch an action</h3>
              ...
              <div className="mt-4 h-24 overflow-y-auto bg-black/40 rounded border border-neutral-800 p-2 text-xs font-mono scroll-smooth" id="action-log">
                {actionLog.map((line, i) => (
                  <div key={i} className={line.startsWith('>') ? 'mb-1 text-neutral-300' : 'text-neutral-600 italic'}>
                    {line}
                  </div>
                ))}
              </div>
